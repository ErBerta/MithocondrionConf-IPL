...
Bitmap *bm_create(int w, int h) {
    SET_ERROR("no error");
    Bitmap *b = malloc(sizeof(Bitmap)); // *b);
    if(!b) {
...
void bm_set_pixel(Bitmap *b, int x, int y, unsigned char R,unsigned char G,unsigned char B) {
    bm_set(b,x,y,bm_rgb(R,G,B));
}

void bm_get_pixel(Bitmap *b, int x, int y, unsigned char * R,unsigned char * G,unsigned char * B) {
    unsigned int col = bm_get(b,x,y);
